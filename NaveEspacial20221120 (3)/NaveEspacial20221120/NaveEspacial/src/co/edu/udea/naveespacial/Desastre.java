...
public class Desastre {
    ...
    public Familia[][] ColisionConAsteroide(Familia[][] familias, int tickMuerte) {
        ...
        for (int k = 0; k < 6; k++) {
            ...
            for (int n = 0; n < familia.size(); n++) {
                ...
                familias[fila][columna].setEstado(false);
                familias[fila][columna].setCausaMuerte("Muerte por Colisión con asteroide");
            }
            columna++;
            if (columna == 5) {
                columna = 0;
    ...
    public Familia[][] fugaDeAire(Familia[][] nave, int tickMuerte) {
       ...
       // int contador = 0;
        for (int index = 0; index < nave.length; index++) {
            for (int j = 0; j < nave[index].length; j++) {
                //contador ++;
                personas = nave[index][j].getFamilia();
                ...
                if (contadorEstado == totalPersonas) {
                    nave[index][j].setEstado(false);
                    nave[index][j].setCausaMuerte("Diferentes eventos antes de llegar a marte");
                    break;
                }
    ...
    public Familia[][] aplicarDesastreLocura(Familia[][] nave, int tickMuerte) {
        ...
        do {
            ...
            if (estadoFamilia) {
                ...
                nave[posicionFila][posicionColumna].setCausaMuerte("Muerte por locura");
                validacion = false;
            }
